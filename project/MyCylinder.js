...
/**
 * MyCylinder
 * Creates a parametric cylinder with configurable dimensions and resolution
 * Used for various cylindrical elements throughout the project such as the helicopter's tail
 */
export class MyCylinder extends CGFobject {
    ...
    initBuffers() {
        ...
        // Generate the top cap (at z = height)
        let topCenterIndex = vertexCount;
        this.vertices.push(0, 0, this.height); // Center of the top cap
        this.normals.push(0, 0, 1); // Normal pointing up
        this.texCoords.push(0.5, 0.5); // Center of the texture
        vertexCount++;

        // Generate the top cap vertices
        for (let i = 0; i <= this.slices; i++) {
            let x = this.radius * Math.cos(i * angle);
            let y = this.radius * Math.sin(i * angle);
        ...
        // Generate the top cap indices (triangles from center to perimeter)
        for (let i = 0; i < this.slices; i++) {
            this.indices.push(
                topCenterIndex, // Center point
                topCenterIndex + i + 1, // Current perimeter point
                topCenterIndex + i + 2 // Next perimeter point
            );
        }
